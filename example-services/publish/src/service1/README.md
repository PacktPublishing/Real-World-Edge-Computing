...
cd example-services
```
#### 2.2 Change to `service1` directory
```
cd publish/src/service1
```
#### 2.3 Build `service1` container image
```
make build
```
#### 2.4 Verify built `service1` container image
```
docker images | grep service1
```
#### 2.5 Push `service1` container image into container registry
```
make push
```
#### 2.6 Publish service
